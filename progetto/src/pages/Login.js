...
const backgroundimageClass =
  "flex h-screen w-full items-center justify-center bg-cover bg-no-repeat";
...
const submitbuttonClass =
  "rounded-3xl bg-blue-400 bg-opacity-50 px-10 py-2 text-white shadow-xl backdrop-blur-md transition-colors duration-300 hover:bg-blue-800";
const cursorpointerClass = "cursor-pointer text-white-500";
const inputtextClass =
  "rounded-3xl border-none bg-blue-400 bg-opacity-50 px-6 py-2 text-center text-inherit placeholder-slate-300 shadow-lg outline-none backdrop-blur-md";
...
async function getArts(user) {
  const artArray = [];
  const artRef = child(dbRef, "/users/" + user.username + "/artworks");
...
async function getEvents(user) {
  const eventArray = [];
  const eventRef = child(dbRef, "/users/" + user.username + "/events");
...
async function getCustomEvents(user) {
  const eventArray = [];
  const eventRef = child(dbRef, "/users/" + user.username + "/customEvents");
