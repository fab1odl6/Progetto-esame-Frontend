...
function ArtShow({
  ...
  const modalClass =
    "fixed inset-0 flex flex-col items-center justify-center w-screen h-screen z-50";
  const containerClass =
    "border-slate-300 border-solid border-4 bg-white overflow-auto";
        ...
        <DialogContent>
          <div className={containerClass}>
            ...
            {artwork.link && (
              <div>
                Source Link:{" "}
                <a className={linkClass} href={artwork.link}>
                  {artwork.title}
                </a>
              </div>
            )}
            ...
            {artwork.classification && (
              <div>Classification: {artwork.classification}</div>
            )}
            <div>
              <button
