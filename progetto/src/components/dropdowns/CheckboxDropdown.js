...
function CheckboxDropdown({ options, title }) {
    ...
    const textClass = "flex justify-between items-center cursor-pointer border rounded p-3 shadow bg-white w-full";
    ...
    return (
        <div ref={divEl} className={containerClass}>
            ...
            {isOpen && <CheckboxDropdownPanel className={panelClass} options={options} />
            }
        </div>
    );
