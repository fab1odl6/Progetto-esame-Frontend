...
function GeneralPanel({ options, title, setIsOpen }) {
  ...
  if (title == "Author") {
    ...
    const handleOptionClick = (option) => {
      ...
      dispatch(
        addFilterItem({
          filterName: "filterSelection",
          valueToAdd: option.label,
        })
      );
    };
    ...
    return (
      <FilterDropdownPanel className={panelClass}>
        {renderedOptions}
      </FilterDropdownPanel>
    );
    ...
    return <CheckboxDropdownPanel className={panelClass} options={options} />;
  }
}
